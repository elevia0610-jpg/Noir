...
// Manifesto Text Reveal Animation
function initManifestoReveal() {
    const textElement = document.querySelector('.about-me-text');
    if (!textElement) return;

    const pairs = textElement.querySelectorAll(".manifesto-pair");
    ...
    ScrollTrigger.create({
        ...
        onEnter: () => {
            pairs.forEach((pair, i) => {
                const block = pair.querySelector(".manifesto-block");
                const word = pair.querySelector(".manifesto-word");

                gsap.timeline({
                    delay: i * 0.18 // slower stagger between words
                })
                .to(block, {
                    scaleX: 0,
                    transformOrigin: "right",
                    duration: 1.1, // slower block animation
                    ease: "power2.inOut"
                })
                .to(word, {
                    opacity: 1,
                    ...
                    duration: 0.9,
                    ease: "power3.out"
                }, "-=0.6"); // Overlap for smooth handoff
            });
        }
    });
}
...
function initCustomCursor() {
    ...
    viewWorkCursor.textContent = 'VIEW';
...
function initViewAllMagnetic() {
    ...
    flowerCursor.innerHTML = `<img src="assets/image/custom-cursor2.png" style="width: 100%; height: 100%; object-fit: contain;">`;
    ...
    function triggerHugAnimation() {
        // Simple scale animation for the image cursor
        gsap.to(flowerCursor, {
            scale: 1.2, // Change this value to adjust size on hover (e.g. 1.3)
            rotation: -10,
            duration: 0.4,
            ease: 'power2.out'
        });
